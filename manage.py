...
#it is for loading command from a configured flask_app
cli = FlaskGroup(create_app=create_app)

#it contains user's information
user_json = [
	{
		"name": "John Doe",
...
#it contains product's information
product_json = [
	{
...
#it is for recreating the database
def recreate_db():
	db.drop_all()
	db.create_all()
	db.session.commit()

#it is for inserting a new code to the database
def seeder():
...
#it is for creating command using flask
@cli.command()
def rsd():
...
#it is for starting the application
